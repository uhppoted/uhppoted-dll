...
bool getDevice() {
  ...
  // clang-format off
    const result resultset[] = {
        ...
        { .field = "IP address",      .type = "string", .value.string.expected = "192.168.1.100",     .value.string.value = d.address },
...
bool setAddress(uint32_t deviceID, const char *address, const char *subnet,
  ...
  if (set_address(DEVICE_ID, "192.168.1.125", "255.255.254.0", "192.168.1.0") !=
      0) {
    printf("ERROR %s\n", errmsg());
    return false;
  }
...
bool getStatus() {
  ...
  // clang-format off
    const result resultset[] = {
        ...
        { .field = "system date/time", .type = "string",  .value.string.expected = "2023-10-06 09:02:48", .value.string.value = s.sysdatetime },
        ...
        { .field = "buttons[1] state", .type = "uint8",   .value.uint8.expected = 0,                      .value.uint8.value = s.buttons[0] },
        { .field = "buttons[2] state", .type = "uint8",   .value.uint8.expected = 1,                      .value.uint8.value = s.buttons[1] },
        ...
        { .field = "event timestamp",  .type = "string",  .value.string.expected = "2023-10-05 12:23:38", .value.string.value  = s.evt.timestamp },
        { .field = "event index",      .type = "uint32",  .value.uint32.expected = 483,                   .value.uint32.value = s.evt.index },
        ...
        { .field = "event card",       .type = "uint32",  .value.uint32.expected = 10058400,              .value.uint32.value = s.evt.card },
