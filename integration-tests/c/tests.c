...
int main(int argc, char **argv) {
  ...
  setup("0.0.0.0:0", "255.255.255.255", "0.0.0.0:60001", 2500, true, &alpha,
        &beta, NULL);
  ...
  if (cmd == NULL || strncmp(cmd, "all", 3) == 0) {
    ...
    for (int i = 0; i < N; i++) {
      ...
      if (strcmp(cmd, t.name) == 0) {
        ok = t.fn();
        goto done; // <evil cackle> always wanted to do this just to annoy
    ...
    if (strcmp(r.type, "uint8") == 0) {
      ...
      if (strcmp(r.value.string.value, r.value.string.expected) != 0) {
        printf("%-21s incorrect %s (expected:%s, got:%s)\n", tag, r.field,
               r.value.string.expected, r.value.string.value);
        ok = false;
      }
    } else {
