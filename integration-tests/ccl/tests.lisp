  ...
  (let ((device (exec #'(lambda (u) (uhppoted-get-device u TEST-DEVICE-ID)))))
    (evaluate "get-device" 
              (list (make-result :field "device ID"              :expected 405419896           :value (device-id      device))
                    (make-result :field "device address"         :expected "192.168.1.100"     :value (device-address device))
  ...
  (let* ((status (exec #'(lambda (u) (uhppoted-get-status u TEST-DEVICE-ID))))
    ...
    (evaluate "get-status" 
              (list (make-result :field "device ID"         :expected 405419896             :value (status-id        status))
                    (make-result :field "system date/time"  :expected "2023-10-06 09:02:48" :value (status-timestamp status))
                    (make-result :field "doors state"       :expected '(t nil nil t)        :value (status-doors     status))
                    (make-result :field "buttons state"     :expected '(nil t t nil)        :value (status-buttons   status))
                    ...
                    (make-result :field "event index"       :expected 483                   :value (event-index      event))
                    (make-result :field "event timestamp"   :expected "2023-10-05 12:23:38" :value (event-timestamp  event))
                    ...
                    (make-result :field "event card number" :expected 10058400              :value (event-card       event))
