...
func getTimeProfile(uu uhppote.IUHPPOTE, profile *C.struct_TimeProfile, deviceID uint32, profileID uint8) error {
	...
	cstring(fmt.Sprintf("%v", p.From), profile.from, 11)
	cstring(fmt.Sprintf("%v", p.To), profile.to, 11)
	...
	cstring(fmt.Sprintf("%v", p.Segments[1].Start), profile.segment1start, 6)
	cstring(fmt.Sprintf("%v", p.Segments[1].End), profile.segment1end, 6)
	cstring(fmt.Sprintf("%v", p.Segments[2].Start), profile.segment2start, 6)
	cstring(fmt.Sprintf("%v", p.Segments[2].End), profile.segment2end, 6)
	cstring(fmt.Sprintf("%v", p.Segments[3].Start), profile.segment3start, 6)
	cstring(fmt.Sprintf("%v", p.Segments[3].End), profile.segment3end, 6)
