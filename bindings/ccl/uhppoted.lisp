  ...
  (multiple-value-bind (date    datep)          (make-heap-ivector 11  '(unsigned-byte 8))
    (unwind-protect
      (rletz ((device (:struct :GoDevice))
        ...
        (let ((ok (external-call "GetDevice" :address       uhppote
...
(defun uhppoted-set-address (uhppote device-id ip-addr subnet-mask gateway-addr) "Sets the controller IP address, subnet mask and gateway"
  ...
  (unwind-protect
    ...
    (rletz ((err  (:struct :GoError) :len     errlen
      ...
      (let ((ok (external-call "SetAddress" :address uhppote 
                                            :unsigned-long device-id 
                                            :address address  
                                            :address subnet
                                            :address gateway
  ...
  (unwind-protect
    (%stack-block ((doors   4)
      ...
      (rletz ((event  (:struct :GoEvent)  :timestamp timestampp)
        ...
        (let ((ok (external-call "GetStatus" :address uhppote 
                                             :unsigned-long device-id 
                                             :address status
                                             :address err
                                             :signed-fullword)))
...
(defun uhppoted-get-time (uhppote device-id) "Retrieves a controller date/time"
  ...
  (unwind-protect
    (rletz ((err (:struct :GoError) :len     errlen
      ...
      (let ((ok (external-call "GetTime" :address uhppote 
                                         :unsigned-long device-id 
                                         :address datetimep
                                         :address err
                                         :signed-fullword)))
        (unless (eq 0 ok) (error 'uhppoted-error :message (%get-cstring errmsgp))))
...
(defun uhppoted-set-time (uhppote device-id datetime) "Sets a controller date/time"
  ...
  (unwind-protect
    ...
    (rletz ((err (:struct :GoError) :len     errlen
                                    :message errmsgp))
      (let ((ok (external-call "SetTime" :address uhppote 
                                         :unsigned-long device-id 
                                         :address dt
                                         :address err
                                         :signed-fullword)))
...
(defun uhppoted-get-listener (uhppote device-id) "Retrieves the controller event listener address"
  ...
  (unwind-protect
    (rletz ((err (:struct :GoError) :len     errlen
                                    :message errmsgp))
      (let ((ok (external-call "GetListener" :address uhppote 
                                             :unsigned-long device-id 
                                             :address listenerp
                                             :address err
                                             :signed-fullword)))
...
(defun uhppoted-set-listener (uhppote device-id listener) "Sets a controller's event listener address and port"
  ...
  (unwind-protect
    ...
    (rletz ((err (:struct :GoError) :len     errlen
                                    :message errmsgp))
      (let ((ok (external-call "SetListener" :address uhppote 
                                             :unsigned-long device-id 
                                             :address addr
                                             :address err
                                             :signed-fullword)))
...
(defun uhppoted-get-door-control (uhppote device-id door) "Retrieves the door control state and open delay for a controller"
  ...
  (unwind-protect
    (rletz ((control (:struct :GoDoorControl))
            (err     (:struct :GoError) :len     errlen
                                        :message errmsgp))
      (let ((ok (external-call "GetDoorControl" :address uhppote 
                                                :unsigned-long device-id 
                                                :unsigned-byte door
                                                :address control 
                                                :address err
...
(defun uhppoted-set-door-control (uhppote device-id door mode delay) "Sets the control mode and delay for a controller door"
  ...
  (unwind-protect
    (rletz ((err (:struct :GoError) :len     errlen
                                    :message errmsgp))
      (let ((ok (external-call "SetDoorControl" :address uhppote 
                                                :unsigned-long device-id 
                                                :unsigned-byte door
                                                :unsigned-byte mode
                                                :unsigned-byte delay
...
(defun uhppoted-open-door (uhppote device-id door) "Remotely opens a controller door"
  ...
  (unwind-protect
    (rletz ((err (:struct :GoError) :len     errlen
                                    :message errmsgp))
      (let ((ok (external-call "OpenDoor" :address uhppote 
                                          :unsigned-long device-id 
                                          :unsigned-byte door
                                          :address err
                                          :signed-fullword)))
...
(defun uhppoted-set-event-index (uhppote device-id index) "Retrieves the current event index from a controller"
  ...
  (unwind-protect
    (rletz ((err (:struct :GoError) :len     errlen
                                    :message errmsgp))
    (with-macptrs ((ok (external-call "SetEventIndex" :address uhppote 
                                                      :unsigned-long device-id 
                                                      :unsigned-long index
                                                      :address err
                                                      :signed-long)))
...
(defun uhppoted-record-special-events (uhppote device-id enabled) "Enables/disables recording additional events for a controller"
  ...
  (unwind-protect
    (rletz ((err (:struct :GoError) :len     errlen
                                    :message errmsgp))
    (with-macptrs ((ok (external-call "RecordSpecialEvents" :address uhppote 
                                                            :unsigned-long device-id 
...
(defun uhppoted-set-pc-control (uhppote device-id enabled) "Enables/disables controller remote access control"
  ...
  (unwind-protect
    (rletz ((err (:struct :GoError) :len     errlen
                                    :message errmsgp))
    (with-macptrs ((ok (external-call "SetPCControl" :address uhppote 
                                                     :unsigned-long device-id 
                                                     :unsigned-byte (if enabled 1 0)
                                                     :address       err
                                                     :signed-long)))
...
(defun uhppoted-set-interlock (uhppote device-id interlock) "Sets a controller interlock mode"
  ...
  (unwind-protect
    (rletz ((err (:struct :GoError) :len     errlen
                                    :message errmsgp))
    (with-macptrs ((ok (external-call "SetInterlock" :address uhppote 
                                                     :unsigned-long device-id 
...
(defun uhppoted-activate-keypads (uhppote device-id reader1 reader2 reader3 reader4) "Activates and deactivates a controller reader access keypads"
  ...
  (unwind-protect
    (rlet ((err (:struct :GoError) :len     errlen
                                    :message errmsgp))
    (with-macptrs ((ok (external-call "ActivateKeypads" :address uhppote 
                                                        :unsigned-long device-id 
...
(defun uhppoted-set-door-passcodes (uhppote device-id door passcode1 passcode2 passcode3 passcode4) "Sets the supervisor keypad passcodes for a door"
  ...
  (unwind-protect
    (rlet ((err (:struct :GoError) :len     errlen
                                    :message errmsgp))
    (with-macptrs ((ok (external-call "SetDoorPasscodes" :address uhppote 
                                                         :unsigned-long device-id 
                                                         :unsigned-byte door
...
(defun uhppoted-restore-default-parameters (uhppote device-id) "Resets a controller to the manufacturer default configuration"
  ...
  (unwind-protect
    (rletz ((err (:struct :GoError) :len     errlen
                                    :message errmsgp))
    (with-macptrs ((ok (external-call "RestoreDefaultParameters" :address uhppote 
                                                                 :unsigned-long device-id 
                                                                 :address       err
                                                                 :signed-long)))
      ; CCL absolutely insists 'err' is a foreign pointer (because with-macptrs maybe ?)
