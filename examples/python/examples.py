...
def commands():
    return {
        ...
        'get-antipassback': {
            'help': "Retrieves the controller anti-passback mode",
            'fn': get_antipassback,
        },
        'set-antipassback': {
...
def set_antipassback(u, args):
    controller = args.controller
    antipassback = args.antipassback

    if antipassback == 'disabled':
        u.set_antipassback(controller, 0)
    elif antipassback == '(1:2);(3:4)':
        u.set_antipassback(controller, 1)
    elif antipassback == '(1,3):(2,4)':
        u.set_antipassback(controller, 2)
    elif antipassback == '1:(2,3)':
        u.set_antipassback(controller, 3)
    elif antipassback == '1:(2,3,4)':
        u.set_antipassback(controller, 4)
    else:
        raise Exception(f"invalid anti-passback mode {antipassback}")
